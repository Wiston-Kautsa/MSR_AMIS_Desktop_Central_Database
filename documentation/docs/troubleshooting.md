...
Run this from a desktop client, replacing `SERVER_IP_OR_NAME`:

```powershell
Invoke-RestMethod http://SERVER_IP_OR_NAME:8090/actuator/health
```

If the server can reach `localhost:8090` but the client cannot reach `SERVER_IP_OR_NAME:8090`, check the server IP/name, network connection, and Windows Firewall.

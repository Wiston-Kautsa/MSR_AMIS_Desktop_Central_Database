    ...
    public AssetHistoryResponse getAssetHistory(String assetCode) {
        ...
        List<AssetHistoryRecordResponse> records = jdbcTemplate.query(
                "SELECT activity_date, event_type, actor, affected_person, details, status " +
                        ...
                        "           4 AS event_order, a.id AS record_id " +
                        "    FROM audit_log a " +
                        ...
                        "           '' AS affected_person, " +
                        "           CASE WHEN COALESCE(TRIM(r.item_condition), '') = '' THEN 'Asset returned' ELSE 'Returned condition: ' || TRIM(r.item_condition) END || " +
                        "           CASE WHEN COALESCE(TRIM(r.previous_condition), '') = '' OR LOWER(TRIM(r.previous_condition)) = LOWER(TRIM(COALESCE(r.item_condition, ''))) THEN '' ELSE ' | Condition changed from: ' || TRIM(r.previous_condition) || ' to: ' || TRIM(COALESCE(r.item_condition, '')) END || " +
                ...
                assetCode, assetCode, assetCode, assetCode, assetCode, assetCode
        );
